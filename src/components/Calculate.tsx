...
async function loadTerrainTriangles(terrainUrn: string | undefined): Promise<Float32Array>{
  // debugger;
  if (!terrainUrn) {
    ...
    return new Float32Array(gltf.scene.children[0].geometry.attributes.position.array)
    // console.log({gltf})
    // return new Float32Array(gltf.scene.children)
  }
}

async function loadTerrain(oldTerrainUrn: string | undefined): Promise<Terrain>{
  ...
  return { mesh: new THREE.Mesh(geometry, material), triangles: terrainTriangles }
}

async function computeElevationDiff(x: number, y: number, oldMesh: Mesh, newMesh: Mesh): Promise<[string, number]>{
  if (oldMesh === newMesh) {
    return [`${x}, ${y}`, 0]
  }
...
export default function CalculateAndStore({ threshold }: Props) {
  const calculateTerrainSteepness = useCallback(async () => {
    ...
    const newTerrainUrn = "urn:adsk-forma-elements:terrain:pro_bteceqejok:db4ff4fa-576a-4958-a635-18323ae008ad:1707828293795"
    const oldTerrainUrn = "urn:adsk-forma-elements:terrain:pro_bteceqejok:db4ff4fa-576a-4958-a635-18323ae008ad:1689843466308"
    ...
    const coordsX = Array.from({ length: diffX }, (_, i) => bBox.minX + i * SCALE)
    const coordsY = Array.from({ length: diffY }, (_, i) => bBox.minY + i * SCALE)

    const fetchPromises = []
    for (const [x, y] of cartesian(coordsX, coordsY)) {
      fetchPromises.push(computeElevationDiff(x, y, newTerrain.mesh, oldTerrain.mesh))
    }

    const result: {[k: string]: number} = Object.fromEntries(await Promise.all(fetchPromises))
